...
func init() {
	var err error
	gopath, err = misc.RunSimple(nil, "go env GOPATH", "")
	if err != nil {
		log.Fatalln(err)
	}
	// Test if executable name gives clue to intended go version
	res := rxBase.FindStringSubmatch(filepath.Base(os.Args[0]))
	if len(res) == 2 {
...
func main() {
	...
	tmpDir, cleanup, err := misc.TempDir()
	if err != nil {
		cleanup()
		log.Fatalln(err)
	}
	...
	err = misc.Build(nil, goEXEPath, outputPath, args, wd)
	if err != nil {
		cleanup()
